  ...
  try {
    ...
    const promises = notification_records.map(async (user_record) => {
      ...
      if (chain_id === 20430 || chain_id === 10200 || chain_id === 84532) {
        nodes = testnet_nodes;
        delegators = testnet_delegators;
      }

      if (chain_id === 2043 || chain_id === 100 || chain_id === 8453) {
        nodes = mainnet_nodes;
        delegators = mainnet_delegators;
      }
      ...
      delegator_data = delegators.filter(
        (delegator) => delegator.nodeId === node_id && delegator.chainId === chain_id && delegator.delegator.toUpperCase() === account.toUpperCase()
      );
      ...
      if (user_record.daily_report === 1) {
        message = await notifications.DailyReport(
          node_data,
          delegator_data,
          node_data_24h
        );
        ...
        console.log(
          `Daily Report has been sent to ${account} for ${node_data[0].tokenName} on ${blockchain}.`
        );
      }
    });
  ...
  try {
    ...
    const promises = notification_records.map(async (user_record) => {
      ...
      if (event_data.length === 0) {
        console.log(
          `No Notifications to send to ${account} for ${node_data[0].tokenName} on ${blockchain}.`
        );
        return;
      }
      ...
      if (user_record.active_status === 1) {
        ...
        console.log(
          `Notifications have been sent to ${account} for ${node_data[0].tokenName} on ${blockchain}.`
        );
      }
    });
